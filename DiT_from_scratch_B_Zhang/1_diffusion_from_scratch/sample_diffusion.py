...
"""从保存点加载"""
checkpoint = torch.load('diffusion_results/diffusion_model_checkpoint_epoch_600.pth', map_location=device, weights_only=True)
...
img_pil.save('generated_image_pokemon_cfg_2.png')


# DDPM采样(普通)  
# x_t 初始化为 [1, 3, 64, 64] 随机值
...
img = (img * 255).astype(np.uint8)
img_pil = Image.fromarray(img)
img_pil.save('generated_image_pokemon_2.png')