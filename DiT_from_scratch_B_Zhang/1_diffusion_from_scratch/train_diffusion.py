...
result_dir = f'result_{datetime.now().strftime("%y%m%d_%H")}'
...
for epoch in range(n_epochs):
    ...
    for batch in train_dataloader:
        ...
        # torch.randint 生成值在 0 到 num_timesteps 之间（不包括 num_timesteps）的随机张量。
        # 生成的张量的形状与 images 张量的第一个维度（batch）相同， (,)括号的写法代表一元元组。
        # 生成的张量被转换为 long 类型，并分配到指定的 device 上。
    ...
    if (epoch + 1) % save_checkpoint_interval == 0:
        torch.save({
            ...
            'val_loss': val_loss,
        }, f'{result_dir}/diffusion_model_checkpoint_epoch_{epoch+1}.pth')
    ...
    if (epoch + 1) % save_checkpoint_interval == 0:
        ...
        with torch.no_grad():
            ...
            for i, img in enumerate(sampled_images):
                ...
                img_pil.save(f'{result_dir}/generated_image_epoch_{epoch+1}_sample_{i}.png')
