...
class Attention(nn.Module):
    ...
    def forward(self, tokens, t=None, context=None):
        B, T, C = tokens.shape  # B: batch size, T: sequence length, C: channels
        H = self.num_heads  
        ...
        # （B, H, T, T）* (B, H, T, head_dim) -> (B, H, T, T)
...
class SpatialTransformer(nn.Module):
    ...
    def forward(self, x, t=None, context=None):
        b, c, h, w = x.shape
        x_res = x  # 用作残差连接
        # x = rearrange(x, "b c h w -> b (h w) c")  # 转换成transformer需要的维度
        x = x.view(b, c, h * w)  # 先将张量重塑为 (b, c, h * w)
        x = x.permute(0, 2, 1)   # 然后调整维度顺序为 (b, h * w, c)

        if context is not None:
            context = self.context_proj(context)

        x = self.transformer(x, t, context)
        # 将形状为 (b, h * w, c) 的张量 x 重排为形状为 (b, c, h, w) 的张量，用于后续Unet需要的维度
        # x = rearrange(x, "b (h w) c -> b c h w", h=h, w=w)
        x = x.view(b, h, w, c)  # 先将张量重塑为 (b, h, w, c)
        x = x.permute(0, 3, 1, 2)  # 然后调整维度顺序为 (b, c, h, w)
        return x + x_res
...
class UNet_Transformer(nn.Module):
    ...
    def get_sinusoidal_position_embedding(self, timesteps, embedding_dim):
        half_dim = embedding_dim // 2   # 256//2 = 128
        emb = math.log(10000) / (half_dim - 1)
        emb = torch.exp(torch.arange(half_dim, device=timesteps.device) * -emb)
        emb = timesteps[:, None] * emb[None, :]    # [batch_size, half_dim] -> [32, 128]
        emb = torch.cat([torch.sin(emb), torch.cos(emb)], dim=-1) # [batch_size, embedding_dim] -> [32, 256]
    ...
    def forward(self, x, t, y):
        ...
        initial_x = x   # [batch, 3, H, W]
        # Ensure y has the correct shape
        if y.dim() == 2:
            y = y.unsqueeze(1)  # 在位置1增加一个维度，[batch, 1, context_dim]

        t = self.get_sinusoidal_position_embedding(t, self.time_dim)  # [batch, embedding_dim]
        t = self.time_mlp(t)

        x1 = self.init_conv(x)      # [batch_size, 64 x H x W]

        x2 = self.down1(x1, t, y)   # [batch_size, 128 x H/2 x W/2]
        x3 = self.down2(x2, t, y)   # [batch_size, 256 x H/4 x W/4]
        x4 = self.down3(x3, t, y)   # [batch_size, 512 x H/8 x W/8]

        x4 = self.middle_block(x4, t, y) # [batch_size, 512 x H/8 x W/8]

        x = self.up1(x4, t, y)         # [batch_size, 512, H/8, W/8] -> [batch_size, 256, H/4, W/4] ##??????????
        x = torch.cat([x, x3], dim=1)  # skip connection 跳跃连接 256 + 256 -> [batch_size, 512, H/4, W/4]
        x = self.up2(x, t, y)          # [batch_size, 128, H/2, W/2]
        x = torch.cat([x, x2], dim=1)  # skip connection 跳跃连接 [batch_size, 256, H/2, W/2]
        x = self.up3(x, t, y)          # [batch_size, 64, H, W]
        x = torch.cat([x, x1], dim=1)  # skip connection 跳跃连接 [batch_size, 128, H, W]

        x = self.final_conv[0](x, t)   # [batch_size, 64, H, W]
        for layer in self.final_conv[1:]:
            x = layer(x)

        return x + initial_x  # 全局残差连接  # [batch_size, 3, H, W]
...
class NoiseScheduler:
    ...
    def cosine_beta_schedule(self, timesteps, s=0.008):
        ...
        参数:
        - timesteps (int): 时间步数，表示扩散过程的步数。
        - s (float): 小偏移量，防止余弦函数在0和1处的梯度爆炸。
    
        返回:
        - torch.Tensor: 在每个时间步上的beta值，形状为(timesteps,)。
        """
        # 计算时间步的总数，+1表示包含开始和结束点
        steps = timesteps + 1   # 1001
        # 在0到timesteps之间线性插值，生成steps个点
        x = torch.linspace(0, timesteps, steps)
        # 计算累积的alphas值，使用余弦函数的平方来得到平滑的减少曲线
        alphas_cumprod = torch.cos(((x / timesteps) + s) / (1 + s) * math.pi * 0.5) ** 2
        # 标准化累积的alphas值，使其初始值为1
        alphas_cumprod = alphas_cumprod / alphas_cumprod[0]
        # 计算beta值，通过相邻时间步的累积alphas值的比率得到，方便后续cumprod累乘
    ...
    def add_noise(self, x_start, t):
        ...
        :return: 添加噪声后的表示
        """
        # 创建 t 的副本，分离计算图，转换为长整型，并移动到与 sqrt_alphas_cumprod 相同的设备上
        t = t.clone().detach().long().to(self.sqrt_alphas_cumprod.device)
        # 生成标准正态分布的噪声
        noise = torch.randn_like(x_start)
        # 获取所需的预计算值
        # view方便后续广播到[batch_size, channels, height, width]
        sqrt_alphas_cumprod_t = self.sqrt_alphas_cumprod[t].view(-1, 1, 1, 1)   # [batch_size, 1, 1, 1]
