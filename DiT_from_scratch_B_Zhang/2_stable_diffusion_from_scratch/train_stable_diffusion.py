...
class AugmentedLatentDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # unsqueeze 是为了将单个图像扩展为批量格式， squeeze再还原
...
dataset = load_dataset("arrow", 
                       data_files={
                           "train": "../data/train/data-00000-of-00001.arrow"
                       },
                       split="train")  # 也可以这样加载本地文件夹的图片数据集
...
# checkpoint = torch.load('../1_diffusion_from_scratch/diffusion_model_final.pth', map_location=device)
...
tokenizer = CLIPTokenizer.from_pretrained("../pretrained_models")
text_encoder = CLIPTextModel.from_pretrained("../pretrained_models").to(device)
...
stable_diffusion_results = f'result_{datetime.now().strftime("%y%m%d_%H")}'
os.makedirs(stable_diffusion_results, exist_ok=True)
...
def diversity_loss(latents, use_cosine=False):
    ...
    if use_cosine:
        ...
        # torch.mm 用于矩阵乘法，latents_norm.t() 是 latents_norm 的转置。
...
print(f"Total training time: {datetime.now()}")
for epoch in range(n_epochs):
    ...
    print(f"Epoch {epoch+1} val loss: {average_val_loss}")
    
    # epoch elapsed time
    epoch_end_time = time.time()
    print(f"Epoch {epoch+1} elapsed time: {epoch_end_time - epoch_start_time}")
    # scheduler.step()
...
print(f"Total_end_time: {datetime.now()}")
print(f"Total training time: {total_end_time - total_start_time}")
# wandb.finish()
